...
if (!moduleSpec || !exportName) {
  process.stderr.write('Usage: postroll-check-env <module-spec> <export-name>\n');
  process.exit(2);
}
...
if (!schema) {
  process.stderr.write(`Export "${exportName}" not found in ${moduleSpec}\n`);
  process.exit(2);
}
...
if (existsSync(envFilePath)) {
  dotenv.config({ override: false, path: envFilePath, quiet: true });
}
