...
export function parseUserAgent(raw: string | null | undefined): ParsedUserAgent {
  ...
  if (!browser && !os) {
    return { browser: null, deviceType, label: 'Unknown device', os: null };
  }
  ...
  const browserLabel = browser
    ...
    : null;
  const label = [browserLabel, os].filter(Boolean).join(' on ') || 'Unknown device';

  return { browser, deviceType, label, os };
}
